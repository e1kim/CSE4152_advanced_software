...
void precalc(int n) {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = i + 2; j <= n; j++) {
				reverse(here.begin() + i, here.begin() + j);	//TODO here의 i부터 j번째까지의 원소 뒤집기(reverse)
				...
				// 배열 원상복구
				reverse(here.begin() + i, here.begin() + j);	//TODO here의 i부터 j번째까지의 원소 뒤집기(reverse), 26번째 줄의 코드와 같음
			}
		}
...
int main() {
	""" Input your loop statement code which contains precalc function """;	// 1<=n<=8 모든 경우에 대한 뒤집기 결과 및 비용 계산
