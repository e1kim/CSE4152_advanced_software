...
void makeGraph(const vector<string>& words) {
    ...
    // TODO모든 원소가 0(int형)인 26 x 26 크기의 2차원 vector 변수 adj 정의
    ...
    for (int j = 1; j < words.size(); ++j) {
        ...
        for (int k = 0; k < len; ++k)
            if (words[i][k] != words[j][k]) {    //TODO 두 단어 words[i]와 words[j]의 k번째 알파벳이 서로 다를 때
...
void dfs(int here) {
    ...
    for (int there = 0; there < adj.size(); there++)
        if (adj[here][there] & !seen[there])   // TODO if (현재 정점에 대한 인접 정점이 있고, 해당 인접 정점을 방문한 적이 없으면)
...
vector<int> topologicalSort() {
    ...
    reverse(order.begin(), order.end());    // TODO vector형 변수 order를 반전(reverse)시킴
