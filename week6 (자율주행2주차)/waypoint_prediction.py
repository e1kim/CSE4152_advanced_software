...
def waypoint_prediction(roadside1_spline, roadside2_spline, num_waypoints=6, way_type = "smooth"):
    ...
    if way_type == "center":
        ...
        print("roadside1 shape: ",roadside1_points.shape)
        print("roadside2 shape: ",roadside2_points.shape)
        ...
        way_points = np.zeros(shape=(2,num_waypoints))
        for i in range(2):
            for j in range(num_waypoints):
                way_points[i][j] = (roadside1_points[i][j] + roadside2_points[i][j] )/2
        ...
        way_points_center = way_points.reshape(num_waypoints*2,)
...
def target_speed_prediction(waypoints, num_waypoints_used=5,
    ...
    target_speed = (max_speed - min_speed ) * np.exp(-exp_constant * np.abs(num_waypoints_used - 2 - curv_center)) + min_speed
